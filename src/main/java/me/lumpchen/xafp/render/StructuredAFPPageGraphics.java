...
public abstract class StructuredAFPPageGraphics implements StructuredAFPGraphics {
	...
	public void drawString(char[] text, float x, float y) {
		...
		if (this.state.textState.font == null) {
			...
			try {
				CodePage codePage = new CodePage(new StructureField(new Introducer()));
				Font font = new DefaultFont(new StructureField(new Introducer()));

				this.state.textState.font = new AFPBitmapFont(codePage, font);
			} catch (IOException exception) {
				logger.severe("Invalid font for {" + new String(text) + "} without font!");
				return;
